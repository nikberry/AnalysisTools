...
void METAnalyser2::analyse(const EventPtr event) {
	analyseMET(event);
...
void METAnalyser2::analyseMET(const EventPtr event) {
	...
	for (unsigned index = 0; index < METAlgorithm::NUMBER_OF_METALGORITHMS; ++index) {
		...
		histMan_->setCurrentHistogramFolder(histogramFolder_ + "/MET" + "/" + prefix);
...
// void METAnalyser2::analyseTransverseMass(const EventPtr event, const ParticlePointer particle) {
...
void METAnalyser2::createHistograms() {
...
//void METAnalyser2::analyse(const EventPtr event, const ParticlePointer particle, const JetCollection jets) {
//	analyse_HT(event, jets);
//	analyse_ST(event, particle, jets);
//	analyse(event, particle);
//}

// void METAnalyser2::analyse_HT(const EventPtr event, const JetCollection jets) {
// 	
// 	histMan_->setCurrentHistogramFolder(histogramFolder_);
// 	weight_ = event->weight() * prescale_ * scale_;
...
// }
// 
// void METAnalyser2::analyse_ST(const EventPtr event, const ParticlePointer particle, const JetCollection jets) {
// 	histMan_->setCurrentHistogramFolder(histogramFolder_);
// 	weight_ = event->weight() * prescale_ * scale_;
// 	for (unsigned index = 0; index < METAlgorithm::NUMBER_OF_METALGORITHMS; ++index) {
// 		std::string prefix = METAlgorithm::prefixes.at(index);
...
METAnalyser2::METAnalyser2(HistogramManagerPtr histMan, std::string histogramFolder) :
...
METAnalyser2::~METAnalyser2() {
}


void METAnalyser2::useTTbarPlusMETSetup(bool use) {
